...
app.post("/tarefas", async (request, response) => {
  try {
    ...
    const taskInDatabase = await Task.findOne({ where: { name: tarefa.name } }); // SELEC FROM taks where name = ''
...
app.put("/tarefas/:id", async (request, response) => {
  try {
    ...
    if (!taskInDatabase) {
      return response.status(404).json({ message: "Tarefa não encontrado" });
    }
...
app.post("/users/login", async (request, response) => {
  try {
    ...
    if (!passwordIsValid) {
      return response
        .status(404)
        .json({ message: "Crendeciais incorreta[password]" });
    }
